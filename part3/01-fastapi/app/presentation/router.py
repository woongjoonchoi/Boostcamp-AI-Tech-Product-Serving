...
from app.application.handlers import get_order_by_id, get_prediction_result, update_order_by_id
...
async def get_order(order_id: UUID, repository: Repository = Depends(get_repository)) -> Union[Order, dict]:
...
async def make_order(
    ...
    background_tasks.add_task(
        get_prediction_result, order_id=new_order.id, model=model, config=config, repository=repository
    )
...
async def update_order(order_id: UUID, order_update: OrderUpdate, repository: Repository = Depends(get_repository)) -> Union[Order, dict]:
    updated_order = update_order_by_id(order_id=order_id, order_update=order_update, repository=repository)
...
async def get_bill(order_id: UUID, repository: Repository = Depends(get_repository)) -> Union[float, dict]:
